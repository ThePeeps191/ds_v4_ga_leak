...
const World = (() => {
  ...
  let planet = null;            // Current planet configuration
  let noiseH, noiseB, noiseD;

  // ---------- Planet configurations ----------
  const PLANET_ARCHETYPES = [
    {
      type: 'Verdant Planet', grass: 0x6fbf4a, leaves: 0x4a9a3a, fog: 0x9fd4ea, sky: 0x87c5ea,
      base: 1, sub: 2, rock: 3, sand: 8, trees: 0.012, plants: 0.02, crystals: 0.004,
      hazardRate: 0.6, hazardName: 'TEMPERATURE 24.1°C · LIGHT BREEZE', ambient: 1.0,
    },
    {
      type: 'Scorched Desert Planet', grass: 0xd8b04a, leaves: 0xb08a3a, fog: 0xf0c890, sky: 0xe8b070,
      base: 8, sub: 8, rock: 12, sand: 8, trees: 0.002, plants: 0.012, crystals: 0.008,
      hazardRate: 2.2, hazardName: 'TEMPERATURE 74.8°C · EXTREME HEAT', ambient: 1.4,
    },
    {
      type: 'Frozen Planet', grass: 0xd8ecf5, leaves: 0xa8d8e8, fog: 0xc8e4f0, sky: 0xa0c8e0,
      base: 1, sub: 3, rock: 3, sand: 8, trees: 0.008, plants: 0.014, crystals: 0.01,
      hazardRate: 2.0, hazardName: 'TEMPERATURE -63.2°C · EXTREME COLD', ambient: 0.8,
    },
    {
      type: 'Crimson Alien Planet', grass: 0xc85a70, leaves: 0xa84a80, fog: 0xd8909a, sky: 0xc07888,
      base: 13, sub: 12, rock: 12, sand: 13, trees: 0.009, plants: 0.018, crystals: 0.007,
      hazardRate: 1.4, hazardName: 'RADIATION 18.4 rad · RADIOACTIVE DUST', ambient: 1.1,
    },
  ];

  const NAME_SYLLABLES = ['Ai', 'Ou', 'Ge', 'No', 'Vi', 'Ta', 'Er', 'Xi', 'Ke', 'La', 'Mu', 'Ze', 'Yin', 'Wu', 'Fei'];
      ...
      for (let z = 0; z < CHUNK; z++) {
        ...
        for (let y = 0; y <= Math.min(h, HEIGHT - 1); y++) {
          ...
          if (y === h) b = planet.base;
            ...
            if (r < 0.015 && y < h - 4) b = 4;        // Magnetized ore
            else if (r < 0.028 && y < h - 4) b = 5;   // Copper ore
            else if (r < 0.036 && y < h - 6) b = 14;  // Cobalt
            // Caves
        ...
        if (h + 1 < HEIGHT) {
          if (surfR < planet.crystals) data[idx(x, h + 1, z)] = 10;                    // Di-hydrogen crystal
          else if (surfR < planet.crystals + planet.plants * 0.5) data[idx(x, h + 1, z)] = 9;  // Sodium flower
          else if (surfR < planet.crystals + planet.plants) data[idx(x, h + 1, z)] = 11;       // Oxygen flower
  ...
  function update(centerX, centerZ, budgetMs = 8) {
    ...
    // Sort by distance, build within time budget
    buildQueue.sort((a, b) => {
  ...
  // Flatten landing area: clear vegetation and compact terrain, return ground height
