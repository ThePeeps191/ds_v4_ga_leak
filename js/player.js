...
const Player = (() => {
  ...
  function die() {
    dead = true;
    UI.notify('LIFE SIGNAL LOST', 'EXOSUIT REBOOTING · SOME RESOURCES HAVE BEEN LOST');
    // Mild penalty: recover but lose half of resources
    setTimeout(() => {
  ...
  function recharge(key) {
    stats[key] = 1;
    if (key === 'hazard') UI.notify('HAZARD PROTECTION CHARGED', 'HAZARD PROTECTION 100%');
    if (key === 'life') UI.notify('LIFE SUPPORT CHARGED', 'LIFE SUPPORT 100%');
    if (key === 'beam') UI.notify('MINING BEAM CHARGED', 'MINING BEAM 100%');
  }
  ...
  function placeBlock() {
    if (Items.count('terrain') <= 0) { Sfx.error(); UI.notify('NO TERRAIN BLOCKS', 'OPEN INVENTORY (TAB) AND CRAFT WITH FERRITE DUST'); return; }
    const eye = new THREE.Vector3(pos.x, pos.y + EYE, pos.z);
