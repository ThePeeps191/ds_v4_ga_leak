// ============ Procedural MC-style 16x16 pixel texture atlas ============
const Textures = (() => {
  ...
  const painters = {
    // 0 Grass top (drawn gray-white, tinted with vertex colors)
    ...
    // 4 Magnetized ore (rock + orange-gold patches)
    4(g) {
      painters[3](g);
      const rng = rngFor(51);
      for (let i = 0; i < 5; i++) {
    ...
    // 11 Di-hydrogen crystal (blue crystal cluster)
    11(g) {
      g.clearRect(0, 0, TILE, TILE);
