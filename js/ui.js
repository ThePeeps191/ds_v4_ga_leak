...
const UI = (() => {
  ...
  function renderCraftList() {
    ...
    Items.RECIPES.forEach(r => {
      ...
      el.innerHTML = `<img src="${Textures.itemIcon(r.out)}">
        ...
        <div class="ci-action">${ok ? 'CRAFT ▸' : 'INSUFFICIENT MATERIALS'}</div>`;
  ...
  const RECHARGES = [
    { key: 'hazard', name: 'Hazard Protection', item: 'sodium', n: 20, icon: '☂' },
    { key: 'life', name: 'Life Support', item: 'oxygen', n: 15, icon: '✚' },
    { key: 'beam', name: 'Mining Beam', item: 'carbon', n: 15, icon: '⚒' },
  ];
  function renderRechargeList() {
    ...
    RECHARGES.forEach(r => {
      ...
      el.innerHTML = `<img src="${Textures.itemIcon(r.item)}">
        ...
        <div class="ci-action">${full ? 'FULLY CHARGED' : has ? 'RECHARGE ▸' : 'INSUFFICIENT'}</div>`;
      el.addEventListener('click', () => {
    ...
    repairTarget.parts.forEach(p => {
      ...
      if (p.fixed) {
        ...
        el.innerHTML = `<img src="${Textures.itemIcon(p.icon)}">
          ...
          <div class="ci-req"><span class="ok">✔ REPAIRED · OPERATIONAL</span></div></div>`;
      } else {
        ...
        el.innerHTML = `<img src="${Textures.itemIcon(p.icon)}">
          ...
          <div class="ci-action">${ok ? 'REPAIR ▸' : 'INSUFFICIENT MATERIALS'}</div>`;
        el.addEventListener('click', () => {
          if (!ok) { Sfx.error(); return; }
          p.req.forEach(([id, n]) => Items.remove(id, n));
          p.fixed = true;
          Sfx.repair();
          notify(p.name + ' REPAIR COMPLETE', 'SYSTEM ONLINE');
          renderRepairList();
